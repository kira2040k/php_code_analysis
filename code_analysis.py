...
class search:
    ...
    def ID(line, line_number):
        ...
        if black_list:
            print(
                colors.color.red(
                    f"found Insecure Deserialization on line {line_number+1}"
                )
            )
            colors.color.reset()
            number_vuln += 1
            ...
            for vuln in black_list:
                ...
                if black_list2:
                    for vuln2 in black_list2:
                        print(
                            colors.color.yellow(f"found SQLi: {vuln}")
                        )  # find SQli but not sure
                        number_vuln += 1
                        SQli_number_vuln += 1
...
class info:
    ...
    def finish():
        ...
        print(
            colors.color.blue(
                check.number_of_vuln("SQLi", check.count_vuln(SQli_number_vuln))
            )
        )
        print(
            colors.color.blue(
                check.number_of_vuln("XSS", check.count_vuln(XSS_number_vuln))
            )
        )
        print(
            colors.color.blue(
                check.number_of_vuln(
                    "execute functions", check.count_vuln(command_injection_number_vuln)
                )
            )
        )
        print(
            colors.color.blue(
                check.number_of_vuln("LFI", check.count_vuln(LFI_number_vuln))
            )
        )
        print(
            colors.color.blue(
                check.number_of_vuln("SSRF", check.count_vuln(SSRF_number_vuln))
            )
        )
        print(
            colors.color.blue(
                check.number_of_vuln(
                    "upload issues", check.count_vuln(check_file_upload_number_vuln)
                )
            )
        )
        print(
            colors.color.blue(
                check.number_of_vuln(
                    ...
                    check.count_vuln(host_header_injection_number_vuln),
                )
            )
        )
        ...
        print(
            colors.color.blue(
                check.number_of_vuln(
                    "insecure deserialization", check.count_vuln(ID_number_vuln)
                )
            )
        )
    ...
    def fix():
        ...
        if SQli_number_vuln != 0:
            ...
            print(
                colors.color.green(
                    "Fix SQLi: https://stackoverflow.com/questions/60174/how-can-i-prevent-sql-injection-in-php"
                )
            )
            colors.color.reset()
        if XSS_number_vuln != 0:
            print(
                colors.color.green(
                    "Fix XSS: https://stackoverflow.com/questions/1996122/how-to-prevent-xss-with-html-php"
                )
            )
            colors.color.reset()
        if LFI_number_vuln != 0:
            print(
                colors.color.green(
                    "Fix LFI: https://security.stackexchange.com/questions/67374/how-to-patch-lfi-vulnerability"
                )
            )
            colors.color.reset()
        if SSRF_number_vuln != 0:
            print(
                colors.color.green(
                    "Fix SSRF: https://stackoverflow.com/questions/35896093/how-can-i-prevent-ssrf-via-pathinfo-passing-a-url-in-php"
                )
            )
            colors.color.reset()
        if check_file_upload_number_vuln != 0:
            print(
                colors.color.green(
                    "Fix file upload: https://www.w3schools.com/php/php_file_upload.asp"
                )
            )
            colors.color.reset()
        if host_header_injection_number_vuln != 0:
            print(
                colors.color.green(
                    "Fix host header injection: https://www.phpcluster.com/host-header-injection-prevention-in-php/"
                )
            )
            colors.color.reset()
...
class server:
    ...
    def scan_files_in_folder(path, original_path):
        ...
        for i2 in folders:
            ...
            if ".php" in i2:
                ...
                payloads = [
                    'x"><img src=adsad onerror=alert(123)//>//',
                    "<svg/onload=alert(1)",
    ...
    def open_server(path):
        ...
        try:
            subprocess.run(
                ["php", "-S", "localhost:2003", "-t", path], capture_output=True
            )
        except:
            print(
                """
